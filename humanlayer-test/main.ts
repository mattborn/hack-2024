...
const openAIHello = async (prompt: string, tools_map: { [key: string]: any }, openai_tools: ChatCompletionTool[]) => {
  ...
  while (response.choices[0].message.tool_calls) {
    ...
    for (const tool_call of tool_calls) {
      ...
      console.log(`calling tools ${tool_name}(${tool_call.function.arguments})`)
